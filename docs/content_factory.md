...
The scheduled GitHub Actions job runs once per day at `17 0 * * *` and uses `DAILY_TARGET=20` by default. Daily target sizing is adaptive: the previous accepted count, rate-limit count, API call count, and errors are saved to `bank/meta.json`, then the next daily run backs off after quota pressure and steps up after clean success. Manual dispatch supports `reset_and_seed`, `seed`, `daily`, and `replace`. The default text generation model is `gemini-3.5-flash`; `GEMINI_MODEL` may override it explicitly, but there is no implicit fallback to older models. When `ENFORCE_GEMINI35=true`, any other model fails the workflow. `tools/assert_refill_result.py` fails CI when a run silently accepts zero items without an explicit quota/rate-limit reason.
